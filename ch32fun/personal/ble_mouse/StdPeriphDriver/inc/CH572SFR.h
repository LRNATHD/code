...
// multi-blocks: __BASE_TYPE__, __CH572SFR_H__, __CH572USBSFR_H__,
// __USB_TYPE__...
...
//
//    SFR:    40000000H - 4000FFFFH,  64KB
//      SYS:    +1000H - 17FFH, include base configuration, interrupt, GPIO,
//      etc...
//    FLASH:    +1800H - 23FFH
//      TMR:    +2400H - 27FFH
//     UART:    +3400H - 37FFH
...
#define R8_SAFE_ACCESS_SIG                                                     \
  (*((PUINT8V)0x40001040)) // WO, safe accessing sign register, must write
                           // SAFE_ACCESS_SIG1 then SAFE_ACCESS_SIG2 to enter
                           // safe accessing mode
#define RB_SAFE_ACC_MODE                                                       \
  0x03 // RO, current safe accessing mode: 11=safe/unlocked (SAM), other=locked
       // (00..01..10..11)
...
#define R8_CHIP_ID                                                             \
  (*((PUINT8V)0x40001041)) // RF, chip ID register, always is ID_CH57*
#define R8_SAFE_ACCESS_ID                                                      \
  (*((PUINT8V)0x40001042)) // RF, safe accessing ID register, always 0x0C
#define R8_WDOG_COUNT                                                          \
  (*((PUINT8V)0x40001043)) // RW, watch-dog count, count by clock frequency
                           // Fsys/131072
...
// #define  RST_FLAG_GPWSM     0x04                      // RO, power on reset
// flag during sleep/shutdown: 0=no power on reset during sleep/shutdown,
// 1=power on reset occurred during sleep/shutdown
...
#define RB_ROM_CODE_WE                                                         \
  0xC0 // RWA, enable flash ROM code area being erase/write: X0=code writing
       // protect, 01=enable 129~240K area program and erase, 11=enable 0~240K
       // area program and erase
#define RB_ROM_CTRL_EN                                                         \
  0x20 // RWA, enable flash ROM control interface enable: 0=disable access,
       // 1=enable access control register
#define RB_ROM_CODE_OFS                                                        \
  0x10 // RWA, code offset address selection in Flash ROM: 0=start address
       // 0x000000, 1=start address 0x008000
#define R8_GLOB_CFG_INFO                                                       \
  (*((PUINT8V)0x40001045)) // RO, global configuration information and status
#define RB_BOOT_LOADER                                                         \
  0x20 // RO, indicate boot loader status: 0=application status (by software
...
#define R8_RST_WDOG_CTRL                                                       \
  (*((PUINT8V)0x40001046)) // RWA, reset and watch-dog control, SAM
#define RB_WDOG_INT_FLAG                                                       \
  0x10 // RW1, watch-dog timer overflow interrupt flag, cleared by RW1 or reload
       // watch-dog count or __SEV(Send-Event)
#define RB_WDOG_INT_EN                                                         \
  0x04 // RWA, watch-dog timer overflow interrupt enable: 0=disable, 1=enable
#define RB_WDOG_RST_EN                                                         \
  0x02 // RWA, enable watch-dog reset if watch-dog timer overflow: 0=as timer
       // only, 1=enable reset if timer overflow
#define RB_BOOT_LOAD_MAN 0x01 // RO, manual boot loader flag, high action
#define RB_SOFTWARE_RESET                                                      \
  0x01 // WA/WZ, global software reset, high action, auto clear
#define R8_GLOB_RESET_KEEP                                                     \
  (*((PUINT8V)0x40001047)) // RW, value keeper during global reset

/* System: clock configuration register */
#define R32_CLOCK_CONFIG                                                       \
  (*((PUINT32V)0x40001008)) // RWA, clock configuration, SAM
#define R8_CLK_SYS_CFG                                                         \
  (*((PUINT8V)0x40001008)) // RWA, system clock configuration, SAM
#define RB_CLK_SYS_MOD                                                         \
  0xC0 // RWA, system clock source mode: 00/10=divided from 32MHz, 01=divided
       // from PLL-600MHz,11=directly from LSI
#define RB_CLK_PLL_DIV 0x1F // RWA, output clock divider from PLL or CK32M
#define R8_HFCK_PWR_CTRL                                                       \
  (*((PUINT8V)0x4000100A))  // RWA, power configuration for system high clock,
                            // SAM
#define RB_CLK_PLL_PON 0x10 // RWA, PLL power control
#define RB_CLK_XT32M_KEEP                                                      \
  0x08 // RWA, RWA, disable auto closing when in halt mode
#define RB_CLK_XT32M_PON 0x04 // RWA, extern 32MHz HSE power contorl

// FckLSI = RC_LSI
// Fpll = XT_32MHz * 18.75 = 600MHz
// Fsys = RB_CLK_SYS_MOD==3 ? FckLSI : ( ( RB_CLK_SYS_MOD[0] ? Fpll : XT_32MHz )
// / RB_CLK_PLL_DIV ) default: Fsys = XT_32MHz / RB_CLK_PLL_DIV = 32MHz / 5
// = 6.4MHz
...
#define R8_SLP_CLK_OFF0                                                        \
  (*((PUINT8V)0x4000100C))       // RWA, sleep clock off control byte 0, SAM
...
#define R8_SLP_CLK_OFF1                                                        \
  (*((PUINT8V)0x4000100D))     // RWA, sleep clock off control byte 1, SAM
...
#define RB_GPIO_WAKE_MODE                                                      \
  0x80 // RWA, GPIO wakeup mode: RB_SLP_GPIO_EDGE_MODE=1,1=all edge ,
       // RB_SLP_GPIO_EDGE_MODE=0,1=rise edge;0=high level
#define RB_WAKE_EV_MODE                                                        \
  0x40 // RWA, event wakeup mode: 0=event keep valid for long time, 1=short
       // pulse event
...
#define RB_SLP_GPIO_EDGE_MODE                                                  \
  0x04 // RWA, GPIO waking edge select:1=all edge,0=pos edge
#define RB_SLP_ENC_WAKE 0x02 // RWA, enable encoder waking
#define RB_SLP_USB_WAKE 0x01 // RWA, enable USB waking
#define R8_SLP_POWER_CTRL                                                      \
  (*((PUINT8V)0x4000100F)) // RWA, peripherals power down control, SAM
#define RB_RAM_RET_LV                                                          \
  0xC0 // RWA, SRAM retention voltage selection: 00=disable, 01=low power mode
       // 1, 10=low power mode 2, 11=low power mode 3,
#define RB_SLP_CLK_RAMX                                                        \
  0x10 // RWA, 1=close main SRAM clock, 0=enable main SRAM clock
#define RB_WAKE_DLY_MOD                                                        \
  0x07 // RWA, wakeup wait time selection,FCLK + 48cycle == SCLK
...
#define R16_PIN_ALTERNATE                                                      \
  (*((PUINT16V)0x40001018)) // RW, function pin alternate configuration low word
#define RB_PIN_DEBUG_EN 0x4000 // RW, debug interface enable
#define RB_PIN_USB_EN                                                          \
  0x2000 // RW, USB analog I/O enable: 0=analog I/O disable, 1=analog I/O enable
#define RB_UDP_PU_EN                                                           \
  0x1000 // RW, USB UDP internal pullup resistance enable: 0=enable/disable by
         // RB_UC_DEV_PU_EN, 1=enable pullup, replace RB_UC_DEV_PU_EN under
         // sleep mode
#define RB_PA_DI_DIS 0x0FFF // RW, 1=disable PA input , 0=enable PA input
#define R16_PIN_ALTERNATE_H                                                    \
  (*((PUINT16V)0x4000101A)) // RW, function pin alternate configuration high
                            // word
...
#define R16_SLP_WAKE_CFG                                                       \
  (*((PUINT16V)0x4000101E)) // RWA, sleep clock aux register, SAM
#define RB_ACAUTO_ENABLE                                                       \
  0x0100 // RWA, 1=enable safe register acess auto off,1=disable safe register
         // acess auto off
#define RB_PRECLK_CNT_SEL                                                      \
  0x0060 // RWA, preclk count value sel,11=2048,101024,01= 512,00=256(actual
         // time value = cnt * Fsys)
#define RB_PRECLK_CNT_EN                                                       \
  0x0010 // RWA, 1=need to wait until reach to wait time to release flck when
         // wake up
#define RB_OSCCLK_RDY_KEEP                                                     \
  0x0001 // RWA, 1=force OSC READY when halt sleep , 0= OSC not READY when halt
         // sleep
#define R8_SLP_CLK_OFF2                                                        \
  (*((PUINT8V)0x4000101D))    // RWA, sleep clock off control byte 2, SAM
...
#define RB_LONG_TIM_SEL                                                        \
  0x06                      // RW, long reset time value selecet
                            // 11=32.768ms,10=25.000ms,01=20.000msm,00=15.000ms
#define RB_LONG_RST_EN 0x01 // RW, long reset enable

/* System: power management register */
#define R32_POWER_MANAG                                                        \
  (*((PUINT32V)0x40001020)) // RWA, power management register, SAM
#define R16_POWER_PLAN                                                         \
  (*((PUINT16V)0x40001020)) // RWA, power plan before sleep instruction, SAM
#define RB_PWR_PLAN_EN                                                         \
  0x8000 // RWA/WZ, power plan enable, auto clear after sleep executed
#define RB_XT_PRE_EN                                                           \
  0x4000 // RWA, extern 32MHz HSE early wake up enable, must be used with LSI
#define RB_PWR_MUST_0 0x2000 // RWA, must write 0
// #define  RB_XT_PRE_CFG      0x1800                    // RWA, extern 32MHz
// HSE early wake up time configuration
#define RB_PWR_LDO5V_EN                                                        \
  0x0100 // RWA, internal LDO5v enable,1=power by V5,0=power by VDD33
...
#define R16_AUX_POWER_ADJ                                                      \
  (*((PUINT16V)0x40001022))  // RWA, aux power adjust control, SAM
...
#define R32_BATTERY_CTRL                                                       \
  (*((PUINT32V)0x40001024)) // RWA, battery voltage detector, SAM
#define R8_BAT_DET_CTRL                                                        \
  (*((PUINT8V)0x40001024)) // RWA, battery voltage detector control, SAM
#define RB_BAT_LOW_IE 0x08 // RWA, interrupt enable for battery low voltage
#define RB_BAT_MON_EN                                                          \
  0x02 // RWA, battery voltage monitor enable under sleep mode
#define RB_PWR_LDO_EN 0x01 // RWA, enable LDO
// request NMI interrupt if both RB_BAT_LOWER_IE and RB_BAT_LOW_IE enabled
#define R8_BAT_DET_CFG                                                         \
  (*((PUINT8V)0x40001025)) // RWA, battery voltage detector configuration, SAM
#define RB_BAT_LOW_VTH                                                         \
  0x03 // RWA, select detector/monitor threshold voltage of battery voltage low
#define R8_BAT_STATUS (*((PUINT8V)0x40001026)) // RO, battery status
#define RB_BAT_STAT_LOW                                                        \
  0x02 // RO, battery low voltage status for detector/monitor, high action

/* System: LSI oscillator control register */
#define R32_OSC_LSI_CTRL                                                       \
  (*((PUINT32V)0x4000102C)) // RWA, LSI oscillator control, SAM
#define R16_INT_LSI_TUNE                                                       \
  (*((PUINT16V)0x4000102C))    // RWA, LSI oscillator tune control, SAM
...
#define R8_RTC_FLAG_CTRL                                                       \
  (*((PUINT8V)0x40001030))    // RW, RTC flag and clear control
#define RB_RTC_TRIG_FLAG 0x80 // RO, RTC trigger action flag
#define RB_RTC_TMR_FLAG 0x40  // RO, RTC timer action flag
#define RB_RTC_TRIG_CLR                                                        \
  0x20 // RW, set 1 to clear RTC trigger action flag, auto clear
#define RB_RTC_TMR_CLR                                                         \
  0x10 // RW, set 1 to clear RTC timer action flag, auto clear
#define R8_RTC_MODE_CTRL (*((PUINT8V)0x40001031)) // RWA, RTC mode control, SAM
#define RB_RTC_LOAD_HI                                                         \
  0x80 // RWA, set 1 to load RTC count high word R32_RTC_CNT_DAY, auto clear
       // after loaded
#define RB_RTC_LOAD_LO                                                         \
  0x40 // RWA, set 1 to load RTC count low word R32_RTC_CNT_LSI, auto clear
       // after loaded
#define RB_RTC_TRIG_EN 0x20 // RWA, RTC trigger mode enable
#define RB_RTC_TMR_EN 0x10  // RWA, RTC timer mode enable
#define RB_RTC_IGNORE_B0                                                       \
  0x08 // RWA, force ignore bit0 for trigger mode: 0=compare bit0, 1=ignore bit0
#define RB_RTC_TMR_MODE                                                        \
  0x07 // RWA, RTC timer mode(unit=cycle): 000: 1; 001: 4096; 010: 12288; 011:
       // 28672 ; 100:61440 ;101: 122880 ;110: 245760; 111: 491520
...
#define R16_RTC_CNT_DIV1                                                       \
  (*((PUINT16V)0x4000103A)) // RO, RTC count based 65536 frequency division
                            // period
#define R32_RTC_CNT_DIV2                                                       \
  (*((PUINT32V)0x4000103C)) // RO, RTC count based 2764800000 frequency division
                            // period

/*System: Miscellaneous Control register */
#define R32_MISC_CTRL                                                          \
  (*((PUINT32V)0x40001048)) // RWA, miscellaneous control register
#define R8_PLL_CONFIG                                                          \
  (*((PUINT8V)0x4000104B))  // RWA, PLL configuration control, SAM
#define RB_PLL_CFG_DAT 0x7F // RWA, PLL configuration control, SAM

/* System: 32MHz oscillator control register */
#define R8_XT32M_TUNE                                                          \
  (*((PUINT8V)0x4000104E)) // RWA, external 32MHz oscillator tune control, SAM
#define RB_XT32M_C_LOAD                                                        \
  0x70 // RWA, external 32MHz oscillator load capacitor tune: Cap =
       // RB_XT32M_C_LOAD * 2 + 10pF
#define RB_XT32M_I_BIAS                                                        \
  0x03 // RWA, external 32MHz oscillator bias current tune: 00=75% current,
       // 01=standard current, 10=125% current, 11=150% current

/* System: oscillator frequency calibration register */
#define R32_OSC_CALIB                                                          \
  (*((PUINT32V)0x40001050)) // RWA, oscillator frequency calibration, SAM
#define R16_OSC_CAL_CNT                                                        \
  (*((PUINT16V)0x40001050)) // RO, system clock count value for LSI multi-cycles
#define RB_OSC_CAL_IF                                                          \
  0x8000 // RW1, interrupt flag for oscillator capture end, set 1 to clear
#define RB_OSC_CAL_OV_CLR                                                      \
  0x4000 // RW1, indicate R8_OSC_CAL_OV_CNT not zero, set 1 to clear
         // R8_OSC_CAL_OV_CNT
#define RB_OSC_CAL_CNT                                                         \
  0x3FFF // RO, system clock count value for LSI multi-cycles
#define R8_OSC_CAL_OV_CNT                                                      \
  (*((PUINT8V)0x40001052)) // RO, oscillator frequency calibration overflow
                           // times
#define R8_OSC_CAL_CTRL                                                        \
  (*((PUINT8V)0x40001053)) // RWA, oscillator frequency calibration control, SAM
#define RB_CNT_CLR 0x80    // RWA,  reset RB_OSC_CAL_CNT
#define RB_OSC_CNT_END                                                         \
  0x40 // RWA, select oscillator capture end mode: 0=normal, 1=append 2 cycles
...
#define RB_OSC_CNT_HALT                                                        \
  0x08 // RO, calibration counter halt status: 0=counting, 1=halt for reading
       // count value
...
/*system: CMP*/
#define R32_CMP_CTRL                                                           \
  (*((PUINT32V)0x40001054)) // RW, configuration for comparator,
#define R8_CMP_CTRL_0                                                          \
  (*((PUINT8V)0x40001054)) // RW, configuration for comparator0,
#define RB_CMP_NREF_LEVEL                                                      \
  0xF0 // RW, comparator negative end point Vref sel:1111=800mv,0000=50mv
#define RB_CMP_SW                                                              \
  0x0C // RW, [0]comparator -channel input sel:1=PA7,0=PA3 ; [1]comparator
       // +channel input sel: 1=COMP_VERF,0=PA2
#define RB_CMP_CAP 0x02 // RW, connect COMP_output to be TIM_cap1_input
#define RB_CMP_EN 0x01  // RW, enable comparators
#define R8_CMP_CTRL_1                                                          \
  (*((PUINT8V)0x40001055)) // RW, configuration for comparator1,
#define RB_CMP_OUT_SEL                                                         \
  0x0C // RW, comparator output sel:11=rise edge,10=fall edge,01=low,00=high
#define RB_CMP_IE 0x01 // RW, comparator interupt enable
#define R8_CMP_CTRL_2                                                          \
  (*((PUINT8V)0x40001056)) // RW, configuration for comparator2,
#define RB_CMP_IF 0x01     // RW1Z, comparator interupt flag
#define R8_CMP_CTRL_3                                                          \
  (*((PUINT8V)0x40001057))   // RW, configuration for comparator3,
...
#define R32_SAFE_ACCESS_SIG2                                                   \
  (*((PUINT32V)0x40001058))    // RO, safe accessing sign register2,
#define RB_FUN_MODE 0x07000000 // RO, function enable
#define RB_FLASH_HALTED                                                        \
  0x00800000 // RO, 2wire flash prohibited operation flag,1=prohibit operation;
             // 0=allow operation
#define RB_MANU_CFG_LOCK                                                       \
  0x00400000 // RO, vendor configuration word lock flag,1=locked; 0= not locked
#define RB_RD_PROTECT                                                          \
  0x00200000 // RO, flash read protecet flag,1=enable read project; 0=disable
             // read project
#define RB_SAFE_AC_DIS                                                         \
  0x00100000 // RO, safe register auto disable flag,1=disable safe access auto
             // off; 0=enable safe access auto off

/* System: Flash ROM control register */
#define R32_FLASH_DATA (*((PUINT32V)0x40001800)) // RO/WO, flash ROM data
#define R32_FLASH_CONTROL                                                      \
  (*((PUINT32V)0x40001804)) // RWA, flash ROM control,byte1 and byte3 need RWA
...
#define R8_FLASH_CFG                                                           \
  (*((PUINT8V)0x40001807)) // RWA, flash ROM access config, SAM
...
#define R32_GPIO_INT_MODE                                                      \
  (*((PUINT32V)0x40001094)) // RW, GPIO interrupt mode: 0=level action, 1=edge
                            // action
#define R16_PA_INT_MODE                                                        \
  (*((PUINT16V)0x40001094)) // RW, GPIO PA interrupt mode: 0=level action,
                            // 1=edge action
#define R16_PA_INT_EDGE_TYPE                                                   \
  (*((PUINT16V)0x40001096)) // RW, GPIO interrupt edge mode: 0=according to por,
                            // 1= don't according to por
...
/* GPIO PA register */
#define R32_PA_DIR                                                             \
  (*((PUINT32V)0x400010A0)) // RW, GPIO PA I/O direction: 0=in, 1=out
...
#define R32_PA_CLR                                                             \
  (*((PUINT32V)0x400010AC)) // WZ, GPIO PA clear output: 0=keep, 1=clear
...
#define R32_PA_PU                                                              \
  (*((PUINT32V)0x400010B0)) // RW, GPIO PA pullup resistance enable
#define R8_PA_PU_0                                                             \
  (*((PUINT8V)0x400010B0)) // RW, GPIO PA pullup resistance enable byte 0
#define R8_PA_PU_1                                                             \
  (*((PUINT8V)0x400010B1)) // RW, GPIO PA pullup resistance enable byte 1
#define R32_PA_PD_DRV                                                          \
  (*((PUINT32V)0x400010B4)) // RW, PA pulldown for input or PA driving
                            // capability for output
#define R8_PA_PD_DRV_0                                                         \
  (*((PUINT8V)0x400010B4)) // RW, PA pulldown for input or PA driving capability
                           // for output byte 0
#define R8_PA_PD_DRV_1                                                         \
  (*((PUINT8V)0x400010B5)) // RW, PA pulldown for input or PA driving capability
                           // for output byte 1
#define R32_PA_SET                                                             \
  (*((PUINT32V)0x400010B8)) // WZ, PA set high for output ,1=set output
                            // high,0=IDLE
/* KEYSCAN register */
#define R16_KEY_SCAN_CTRL (*((PUINT16V)0x40001064)) // KEY SCAN control register
#define RB_CLR_WAKEUP_EN                                                       \
  0x4000 // RW, claer wake_up siginal after chip wakeing up, 1=enable 0=disable
#define RB_SCAN_1END_WAKE_EN                                                   \
  0x2000 // RW, wake up chip after 1 round of key scanning, 1=enable 0=disable
#define RB_PIN_SCAN_EN                                                         \
  0x1F00 // RW, select which pin could be scaned, 1=enable 0=disable
...
#define R8_KEY_SCAN_INT_EN                                                     \
  (*((PUINT8V)0x40001066))     // KEY SCAN interupt enable register
...
#define R8_KEY_SCAN_INT_FLAG                                                   \
  (*((PUINT8V)0x40001067))     // KEY SCAN interupt flag register
...
#define R32_KEY_SCAN_NUMB                                                      \
  (*((PUINT32V)0x40001068))       // SCAN_KEY number address now register
...
#define R32_TMR_CNT_END                                                        \
  (*((PUINT32V)0x4000240C)) // RW, TMR end count value, only low 26 bit
#define R32_TMR_FIFO                                                           \
  (*((PUINT32V)0x40002410)) // RO/WO, TMR FIFO register, only low 26 bit
...
// #define  RB_WAKEUP_CLR_EN   0x10                      // RW, clear wake_up
// siginal after chip wake up enable
#define RB_ENC_DIR 0x20 // RO, encoder director,0=forward,1=backward
#define RB_RD_CLR_EN                                                           \
  0x08 // RW, clear encoder count value after R32_ENC_REG_CCNT be read
...
#define R8_ENC_INTER_EN                                                        \
  (*((PUINT8V)0x40002421)) // RW, ENCODER interupt enable register
#define RB_IE_DIR_DEC 0x02 // RW, encode decrease interupt enable
#define RB_IE_DIR_INC 0x01 // RW, encode increase interupt enable
#define R8_ENC_INT_FLAG                                                        \
  (*((PUINT8V)0x40002422)) // RW, ENCODER interupt flag register
...
#define RB_TMR_PWM_REPEAT                                                      \
  0xC0 // RW, timer PWM repeat mode: 00=1, 01=4, 10=8, 11-16
#define RB_TMR_CAP_EDGE                                                        \
  0xC0 // RW, timer capture edge mode: 00=disable, 01=edge change, 10=fall to
       // fall, 11-rise to rise
#define RB_TMR_OUT_POLAR                                                       \
  0x10 // RW, timer PWM output polarity: 0=default low and high action,
       // 1=default high and low action
#define RB_TMR_CAP_COUNT                                                       \
  0x10 // RW, count sub-mode if RB_TMR_MODE_IN=1: 0=capture, 1=count
...
#define RB_TMR_IE_DMA_END                                                      \
  0x08 // RW, enable interrupt for timer1/2 DMA completion
#define RB_TMR_IE_FIFO_HF                                                      \
  0x04 // RW, enable interrupt for timer FIFO half (capture fifo >=4 or PWM fifo
       // <=3)
#define RB_TMR_IE_DATA_ACT                                                     \
  0x02 // RW, enable interrupt for timer capture input action or PWM trigger
#define RB_TMR_IE_CYC_END                                                      \
  0x01 // RW, enable interrupt for timer capture count timeout or PWM cycle end
#define TMR_INT_FLAG 6
#define RB_TMR_IF_FIFO_OV 0x10 // RW1, interrupt flag for timer FIFO overflow
#define RB_TMR_IF_DMA_END                                                      \
  0x08 // RW1, interrupt flag for timer1/2 DMA completion
#define RB_TMR_IF_FIFO_HF                                                      \
  0x04 // RW1, interrupt flag for timer FIFO half (capture fifo >=4 or PWM fifo
       // <=3)
#define RB_TMR_IF_DATA_ACT                                                     \
  0x02 // RW1, interrupt flag for timer capture input action or PWM trigger
#define RB_TMR_IF_CYC_END                                                      \
  0x01 // RW1, interrupt flag for timer capture count timeout or PWM cycle end
...
#define R8_UART_IIR                                                            \
  (*((PUINT8V)0x40003404)) // RO, UART interrupt identification
#define R8_UART_LSR (*((PUINT8V)0x40003405)) // RO, UART line status
#define R8_UART_RBR                                                            \
  (*((PUINT8V)0x40003408)) // RO, UART receiver buffer, receiving byte
#define R8_UART_THR                                                            \
  (*((PUINT8V)0x40003408)) // WO, UART transmitter holding, transmittal byte
...
#define R8_UART_DIV                                                            \
  (*((PUINT8V)0x4000340E)) // RW, UART pre-divisor latch byte, only low 7 bit,
                           // from 1 to 0/128

/* UART register address offset and bit define */
#define UART_FIFO_SIZE 8 // UART FIFO size (depth)
#define UART_RECV_RDY_SZ                                                       \
  7 // the max FIFO trigger level for UART receiver data available
...
#define RB_FCR_FIFO_TRIG                                                       \
  0xC0 // RW, UART receiver FIFO trigger level: 00-1byte, 01-2bytes, 10-4bytes,
       // 11-7bytes
...
#define RB_IIR_NO_INT                                                          \
  0x01 // RO, UART no interrupt flag: 0=interrupt action, 1=no interrupt
#define UART_LSR 5
...
#define R8_SPI_CTRL_CFG                                                        \
  (*((PUINT8V)0x40004001)) // RW, SPI configuration control
#define R8_SPI_INTER_EN (*((PUINT8V)0x40004002)) // RW, SPI interrupt enable
#define R8_SPI_CLOCK_DIV                                                       \
  (*((PUINT8V)0x40004003)) // RW, SPI master clock divisor
...
#define R32_SPI_INTER_CFG1                                                     \
  (*((PUINT32V)0x40004008)) // RO, SPI interrupt configuration1
#define R16_SPI_TOTAL_CNT                                                      \
  (*((PUINT16V)0x4000400C)) // RW, SPI total byte count, only low 12 bit
...
#define RB_SPI_FIFO_DIR                                                        \
  0x10 // RW, SPI FIFO direction: 0=out(write @master mode), 1=in(read @master
       // mode)
#define RB_SPI_SLV_CMD_MOD                                                     \
  0x08 // RW, SPI slave command mode: 0=byte stream, 1=first byte command
#define RB_SPI_MST_SCK_MOD 0x08 // RW, SPI master clock mode: 0=mode 0, 1=mode 3
#define RB_SPI_2WIRE_MOD                                                       \
  0x04 // RW, SPI enable 2 wire mode for slave: 0=3wire(SCK0,MOSI,MISO),
       // 1=2wire(SCK0,MISO=MXSX)
#define RB_SPI_ALL_CLEAR 0x02 // RW, force clear SPI FIFO and count
#define RB_SPI_MODE_SLAVE                                                      \
  0x01 // RW, SPI slave mode: 0=master/host, 1=slave/device
#define SPI_CTRL_CFG 1
#define RB_SPI_MST_DLY_EN 0x40 // RW, SPI master input delay enable
#define RB_SPI_BIT_ORDER                                                       \
  0x20 // RW, SPI bit data order: 0=MSB first, 1=LSB first
#define RB_SPI_AUTO_IF                                                         \
  0x10 // RW, enable buffer/FIFO accessing to auto clear RB_SPI_IF_BYTE_END
       // interrupt flag
#define RB_SPI_DMA_LOOP 0x04 // RW, SPI DMA address loop enable
#define RB_MST_CLK_SEL                                                         \
  0x02 // RW, hclk polarity reversal,1= polarity reversal,0=IDLE
#define RB_SPI_DMA_ENABLE 0x01 // RW, SPI DMA enable
#define SPI_INTER_EN 2
#define RB_SPI_IE_FST_BYTE                                                     \
  0x80 // RW, enable interrupt for SPI slave mode first byte received
...
#define RB_SPI_IE_CNT_END                                                      \
  0x01 // RW, enable interrupt for SPI total byte count end
...
#define RB_SPI_IF_FST_BYTE                                                     \
  0x80 // RW1, interrupt flag for SPI slave mode first byte received
...
#define RB_SPI_IF_FIFO_HF                                                      \
  0x04 // RW1, interrupt flag for SPI FIFO half (RB_SPI_FIFO_DIR ? >=4bytes :
       // <4bytes)
#define RB_SPI_IF_BYTE_END 0x02 // RW1, interrupt flag for SPI byte exchanged
#define RB_SPI_IF_CNT_END                                                      \
  0x01 // RW1, interrupt flag for SPI total byte count end
...
#define R16_I2C_OADDR1                                                         \
  (*((PUINT16V)0x40004808)) // RW, I2C own address register 1
#define R16_I2C_OADDR2                                                         \
  (*((PUINT16V)0x4000480C)) // RW, I2C own address register 2
...
// #define R8_I2C_PEC          (*((PUINT8V) 0x40004819)) // R0, I2C Packet error
// checking register
#define R16_I2C_CKCFGR                                                         \
  (*((PUINT16V)0x4000481C))                   // RW, I2C clock control register
...
#define RB_I2C_ALERT                                                           \
  0x2000 // RW, SMBus alert: 0=Releases SMBA pin high, 1=Drives SMBA pin low.
#define RB_I2C_PEC                                                             \
  0x1000 // RW, Packet error checking: 0=No PEC transfer, 1=PEC transfer (in Tx
         // or Rx mode)
#define RB_I2C_POS 0x0800 // RW, Acknowledge/PEC Position (for data reception)
#define RB_I2C_ACK 0x0400 // RW, Acknowledge enable
#define RB_I2C_STOP                                                            \
  0x0200 // RW, Stop generation: master mode: 0=no stop, 1=stop after the
         // current byte transfer or after the current Start condition is sent;
         // slave mode: 0=no stop, 1=Release the SCL and SDA lines after the
         // current byte transfer
#define RB_I2C_START                                                           \
  0x0100 // RW, Start generation: master mode: 0=no start, 1=repeated start;
...
#define RB_I2C_FREQ                                                            \
  0x003F // RW, Peripheral clock frequency, The minimum allowed frequency is 2
         // MHz,the maximum frequency is 36 MHz
#define I2C_OADDR1 8
#define RB_I2C_ADDMODE                                                         \
  0x8000 // RW, Addressing mode (slave mode): 0=7-bit slave address, 1=10-bit
         // slave address
#define RB_I2C_ADD9_8                                                          \
  0x0300 // RW, bit[9:8] of address in 10-bit addressing mode
...
#define RB_I2C_ADDR                                                            \
  0x0002 // RW0, Address sent (master mode)/matched (slave mode) flag
#define RB_I2C_SB 0x0001 // RW0, Start bit flag (Master mode)
#define I2C_STAR2 24
#define RB_I2C_PECX 0xFF00 // RO, Packet error checking register
#define RB_I2C_DUALF                                                           \
  0x0080 // RO, Dual flag (Slave mode): 0=Received address matched with OAR1,
         // 1=Received address matched with OAR2
#define RB_I2C_SMBHOST                                                         \
  0x0040 // RO, SMBus host header (Slave mode) received flag
#define RB_I2C_SMBDEFAULT                                                      \
  0x0020 // RO, SMBus device default address (Slave mode) received flag
#define RB_I2C_GENCALL                                                         \
  0x0010 // RO, General call address (Slave mode) received flag
#define RB_I2C_TRA                                                             \
  0x0004 // RO, Trans flag: 0=data bytes received, 1=data bytes transmitted
...
#define RB_I2C_F_S                                                             \
  0x8000 // RW, I2C master mode selection: 0=standard mode, 1=fast mode
#define RB_I2C_DUTY 0x4000 // RW, Fm mode duty cycle: 0=L/H=2, 1=L/H=16/9
#define RB_I2C_CCR                                                             \
  0x0FFF // RW, Controls the SCL clock in Fm/Sm mode (Master mode)
#define I2C_RTR 32
...
#define R16_PWM1_DATA                                                          \
  (*((PUINT16V)0x40005004)) // RW, PWM1 data (16 bit) holding
#define R16_PWM2_DATA                                                          \
  (*((PUINT16V)0x40005006))                   // RW, PWM2 data (16 bit) holding
...
#define R16_PWM3_DATA                                                          \
  (*((PUINT16V)0x40005008)) // RW, PWM3 data (16 bit) holding
#define R32_PWM4_5_DATA (*((PUINT32V)0x40005010)) // RW, PWM4-5 data register
#define R16_PWM4_DATA                                                          \
  (*((PUINT16V)0x40005010)) // RW, PWM4 data (16 bit) holding
#define R16_PWM5_DATA                                                          \
  (*((PUINT16V)0x40005012))                    // RW, PWM5 data (16 bit) holding
...
#define RB_PWM_CYC_PRE                                                         \
  0x02 // RW, select PWM cycle interrupt point: 0=after count 0xFE (0x7E for 7
       // bits mode...), 1=after count 0xF0 (0x70 for 7 bits mode...)
...
#define R16_PWM_CYC_VALUE                                                      \
  (*((PUINT16V)0x40005014)) // RW, PWM1\2\3 cycle value for 16bit
#define R16_PWM_CYC1_VALUE                                                     \
  (*((PUINT16V)0x40005016)) // RW, PWM4\5 cycle value for 16bit
...
#define R32_PWM_DMA_BEG                                                        \
  (*((PUINT32V)0x40005020)) // RW, PWM DMA addr of begining
...
#define PWM_POLAR 1
#define RB_PWM5_POLAR                                                          \
  0x10 // RW, PWM5 output polarity: 0=default low and high action, 1=default
       // high and low action
#define RB_PWM4_POLAR                                                          \
  0x08 // RW, PWM4 output polarity: 0=default low and high action, 1=default
       // high and low action
#define RB_PWM3_POLAR                                                          \
  0x04 // RW, PWM3 output polarity: 0=default low and high action, 1=default
       // high and low action
#define RB_PWM2_POLAR                                                          \
  0x02 // RW, PWM2 output polarity: 0=default low and high action, 1=default
       // high and low action
#define RB_PWM1_POLAR                                                          \
  0x01 // RW, PWM1 output polarity: 0=default low and high action, 1=default
       // high and low action
#define PWM_CONFIG 2
...
#define RB_PWM4_5_STAG_EN                                                      \
  0x08 // RW, PWM4/5 stagger output enable: 0=independent output, 1=stagger
       // output
#define RB_PWM_CYC_MOD                                                         \
  0x06 // RW, PWM data width mode: 00=8 bits data, 01=7 bits data, 10=6 bits
       // data, 11=16 bits data
#define RB_PWM_CYCLE_SEL                                                       \
  0x01 // RW, PWM cycle selection: 0=256/128/64/32 clocks, 1=255/127/63/31
       // clocks
#define PWM_DMA_CTRL 3
#define RB_DMA_SEL                                                             \
  0x04 // RW, RB_PWM_SYN_EN=0: 1=DMA choose 1/2/3 channel output , 0=DMA choose
       // 4/5 channel output ,RB_PWM_SYN_EN=1:  1=DMA choose 1/2/3/4/5 channel
       // output
...
#define BOOT_LOAD_CFG                                                          \
  0x3E000 // start address of configuration information for boot loader program
...
// #define INT_ID_TMR0         0                         // interrupt number for
// Timer0
...
#define INT_ID_WDOG_BAT                                                        \
  35 // interrupt number for Watch-Dog timer and Battery low voltage
#define INT_VEC_ENTRY_SZ 4 // size of each interrupt vector entry
// #define INT_ADDR_TMR0       (INT_ID_TMR0*INT_VEC_ENTRY_SZ+64)    // interrupt
// vector address for Timer0
#define INT_ADDR_GPIO_A                                                        \
  (INT_ID_GPIO_A * INT_VEC_ENTRY_SZ +                                          \
   64) // interrupt vector address for GPIO port A
#define INT_ADDR_SPI                                                           \
  (INT_ID_SPI * INT_VEC_ENTRY_SZ + 64) // interrupt vector address for SPI
#define INT_ADDR_BLEB                                                          \
  (INT_ID_BLEB * INT_VEC_ENTRY_SZ + 64) // interrupt vector address for BLEBB
#define INT_ADDR_BLEL                                                          \
  (INT_ID_BLEL * INT_VEC_ENTRY_SZ + 64) // interrupt vector address for BLELLE
#define INT_ADDR_USB                                                           \
  (INT_ID_USB * INT_VEC_ENTRY_SZ + 64) // interrupt vector address for USB
#define INT_ADDR_TMR                                                           \
  (INT_ID_TMR * INT_VEC_ENTRY_SZ + 64) // interrupt vector address for Timer1
#define INT_ADDR_UART                                                          \
  (INT_ID_UART * INT_VEC_ENTRY_SZ + 64) // interrupt vector address for UART
#define INT_ADDR_RTC                                                           \
  (INT_ID_RTC * INT_VEC_ENTRY_SZ + 64) // interrupt vector address for RTC
#define INT_ADDR_CMP (INT_ID_CMP * INT_VEC_ENTRY_SZ + 64)
#define INT_ADDR_I2C (INT_ID_I2C * INT_VEC_ENTRY_SZ + 64)
#define INT_ADDR_PWMX                                                          \
  (INT_ID_PWMX * INT_VEC_ENTRY_SZ + 64) // interrupt vector address for PWM1~5
#define INT_ADDR_KEYSCAN                                                       \
  (INT_ID_KEYSCAN * INT_VEC_ENTRY_SZ +                                         \
   64) // interrupt vector address for KEYSCAN
#define INT_ADDR_ENCODE                                                        \
  (INT_ID_ENCODE * INT_VEC_ENTRY_SZ +                                          \
   64) // interrupt vector address for ENCODER
#define INT_ADDR_WDOG_BAT                                                      \
  (INT_ID_WDOG_BAT * INT_VEC_ENTRY_SZ +                                        \
   64) // interrupt vector address for Watch-Dog timer and Battery low voltage
...
/*       USB  */
#define R32_USB_CONTROL                                                        \
  (*((PUINT32V)0x40008000)) // USB control & interrupt enable & device address
...
//   RB_PIN_USB_DP_PU=1 or need external pullup resistance 0 1x: enable USB
//   device and enable internal pullup resistance 1 00: enable USB host and
//   normal status 1 01: enable USB host and force UDP/UDM output SE0 state 1
//   10: enable USB host and force UDP/UDM output J state 1 11: enable USB host
//   and force UDP/UDM output resume or K state
#define RB_UC_INT_BUSY                                                         \
...
#define R8_UDEV_CTRL                                                           \
  (*((PUINT8V)0x40008001)) // USB device physical prot control
#define RB_UD_PD_DIS                                                           \
...
#define R32_USB_STATUS                                                         \
  (*((PUINT32V)0x40008004)) // USB miscellaneous status & interrupt flag &
                            // interrupt status
...
#define RB_UMS_SOF_ACT                                                         \
  0x40 // RO, indicate host SOF timer action status for USB host
#define RB_UMS_SIE_FREE 0x20 // RO, indicate USB SIE free status
#define RB_UMS_R_FIFO_RDY                                                      \
  0x10 // RO, indicate USB receiving FIFO ready status (not empty)
#define RB_UMS_BUS_RESET 0x08 // RO, indicate USB bus reset status
#define RB_UMS_SUSPEND 0x04   // RO, indicate USB suspend status
#define RB_UMS_DM_LEVEL                                                        \
  0x02 // RO, indicate UDM level saved at device attached to USB host
#define RB_UMS_DEV_ATTACH                                                      \
  0x01 // RO, indicate device attached status on USB host
...
#define RB_UIS_SETUP_ACT                                                       \
  0x80 // RO, indicate SETUP token & 8 bytes setup request received for USB
       // device mode
#define RB_UIS_TOG_OK 0x40 // RO, indicate current USB transfer toggle is OK
#define RB_UIS_TOKEN1                                                          \
  0x20 // RO, current token PID code bit 1 received for USB device mode
#define RB_UIS_TOKEN0                                                          \
  0x10 // RO, current token PID code bit 0 received for USB device mode
...
// mode, keep last status during SETUP token, clear RB_UIF_TRANSFER (
// RB_UIF_TRANSFER from 1 to 0 ) to set free
...
// bUEPn_RX_EN & bUEPn_TX_EN: USB endpoint 5/6/7 buffer mode, buffer start
// address is UEPn_DMA
...
//   1 1:  64 bytes buffer for receiving (OUT endpoint) + 64 bytes buffer for
//   transmittal (IN endpoint), total=128bytes
...
#define R32_USB_EP1_CTRL                                                       \
  (*((PUINT32V)0x40008024)) // endpoint 1 control & transmittal length
...
#define R32_USB_EP3_CTRL                                                       \
  (*((PUINT32V)0x4000802c)) // endpoint 3 control & transmittal length
...
#define R8_UH_TX_LEN                                                           \
  R8_UEP3_T_LEN // host transmittal endpoint transmittal length
...
#define R32_USB_EP4_CTRL                                                       \
  (*((PUINT32V)0x40008030)) // endpoint 4 control & transmittal length
...
#define R32_USB_EP5_CTRL                                                       \
  (*((PUINT32V)0x40008064)) // endpoint 5 control & transmittal length
...
#define R32_USB_EP6_CTRL                                                       \
  (*((PUINT32V)0x40008068)) // endpoint 6 control & transmittal length
...
#define R32_USB_EP7_CTRL                                                       \
  (*((PUINT32V)0x4000806C)) // endpoint 7 control & transmittal length
...
#define USB_BO_CBW_SIG                                                         \
  0x43425355 /* identification mark of CBW command block 'USBC' */
#define USB_BO_CSW_SIG                                                         \
  0x53425355 /* identification mark of CSW command state block 'USBC'USBS' */
#endif
