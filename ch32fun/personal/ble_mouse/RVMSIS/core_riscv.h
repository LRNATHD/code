/********************************** (C) COPYRIGHT
 * ******************************* File Name          : core_riscv.h Author
 *        : WCH Version            : V1.0.0 Date               : 2024/07/25
 * Description        : CH585 Series RISC-V Core Peripheral Access Layer Header
 * File
 *********************************************************************************
...
__risc_v_enable_irq(uint32_t mpie_mie) {
  uint32_t result;
 ...
 */
__attribute__((always_inline)) RV_STATIC_INLINE uint32_t
__risc_v_disable_irq(void) {
  uint32_t result;

  __asm volatile("csrrc %0, 0x800, %1" : "=r"(result) : "r"(0x88) : "memory");
  return result & 0x88;
}
...
#define PFIC_EnableAllIRQ()                                                    \
  {                                                                            \
    write_csr(0x800, 0x88);                                                    \
  }
#define PFIC_DisableAllIRQ()                                                   \
  {                                                                            \
    write_csr(0x800, 0x80);                                                    \
 ...
 */
__attribute__((always_inline)) RV_STATIC_INLINE void
PFIC_DisableIRQ(IRQn_Type IRQn) {
 ...
 */
__attribute__((always_inline)) RV_STATIC_INLINE uint32_t
PFIC_GetStatusIRQ(IRQn_Type IRQn) {
 ...
 */
__attribute__((always_inline)) RV_STATIC_INLINE uint32_t
PFIC_GetPendingIRQ(IRQn_Type IRQn) {
 ...
 */
__attribute__((always_inline)) RV_STATIC_INLINE void
PFIC_SetPendingIRQ(IRQn_Type IRQn) {
 ...
 */
__attribute__((always_inline)) RV_STATIC_INLINE void
PFIC_ClearPendingIRQ(IRQn_Type IRQn) {
 ...
 */
__attribute__((always_inline)) RV_STATIC_INLINE void
PFIC_SetPriority(IRQn_Type IRQn, uint8_t priority) {
 ...
 */
__attribute__((always_inline)) RV_STATIC_INLINE void
__MCPY(void *dst, void *start, void *end) {
  memcpy(dst, start, (size_t)((char *)end - (char *)start));
}
...
RV_STATIC_INLINE uint32_t SysTick_Config(uint32_t ticks) {
  ...
  SysTick->CTLR = SysTick_CTLR_STRE | SysTick_CTLR_STCLK | SysTick_CTLR_STIE |
