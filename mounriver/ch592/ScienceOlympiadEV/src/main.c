...
#define left_Correction                                                        \
  1 // easily be able to switch motor direction if code is off
...
// buttons all pulled to ground, pullup should be used. SDA/SCL also connect to
// the screen, so it can be funky
...
__INTERRUPT     // this is a interupt
  ...
  if (GPIOB_ReadITFlagBit(M3ENCA)) {
    uint8_t a =
        (GPIOB_ReadPortPin(M3ENCA) != 0); // figure out which encoder was edging
    uint8_t b = (GPIOB_ReadPortPin(Lb) != 0);

    if (a == b)
      cal_count++; // count up/down (up=clockwise)
    else
      cal_count--;

    if (a)
      GPIOB_ITModeCfg(
          La, GPIO_ITMode_FallEdge); // go onto the next closest encoder reading
    else
  ...
  if (GPIOB_ReadITFlagBit(Ra)) {
    uint8_t a =
        (GPIOB_ReadPortPin(Ra) != 0); // figure out which encoder got edged
    uint8_t b = (GPIOB_ReadPortPin(Rb) != 0);

    if (a == b)
      count_m4++; // count up motors spin
    else
      count_m4--;

    if (a)
      GPIOB_ITModeCfg(Ra, GPIO_ITMode_FallEdge); // read for the closest step
    else
      GPIOB_ITModeCfg(Ra, GPIO_ITMode_RiseEdge);
...
__INTERRUPT     // this is a interupt
  ...
  if (GPIOB_ReadITFlagBit(M3ENCA)) {
    ...
    if (a == b)
      count_m3++; // count up/down (up=clockwise)
    else
      count_m3--;
...
int main() {
  ...
  GPIOA_ModeCfg(
      Left_Enable | Left_Phase | Right_Enable | Right_Phase,
      GPIO_ModeOut_PP_5mA); // make the motors bits configured for output
  GPIOB_ModeCfg(Left_Encoder_A | Left_Encoder_B | Right_Encoder_A |
                    Right_Encoder_B,
                GPIO_ModeIN_PU); // make the encoders bits configured for input
                                 // (pulled up)

  GPIOB_ITModeCfg(
      Left_Encoder_A,
      GPIO_ITMode_FallEdge); // setup the interupts, start at falling edge
  GPIOA_ITModeCfg(Right_Encoder_A, GPIO_ITMode_FallEdge);

  rightmotormaxspeed = combinedmaxspeed =
      min(leftmotormaxspeed, rightmotormaxspeed);
...
int speedcalibration() {
  // spin the motors for a little bit at like 1%, 5% 10%, 25%, 50%, 100%.
  // calibrate speed for those stages
  return 1;
}
